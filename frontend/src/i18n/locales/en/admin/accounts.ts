export default {
    accounts: {
      ...
      columns: {
        ...
        upstreamBillingRate: 'Upstream Rate/Balance',
        weight: 'Weight',
      ...
      upstreamBilling: {
        trustWarning: 'The rate, balance, subscription, and usage are declared by the upstream site for the current API key. Sub2API cannot verify that they match actual charges. Verify them against the upstream bill.',
        ...
        billingMode: 'Billing type: {value}',
        billingModeSubscription: 'Subscription billing',
        billingModeBalance: 'Balance billing',
		balanceQueryMode: 'Balance query mode',
		balanceQueryAuto: 'Auto-compatible (Sub2API / generic)',
		balanceQueryGeneric: 'Generic /user/balance',
		balanceQueryNewAPI: 'New API /api/user/self',
		balanceQueryCustom: 'Custom JSON template',
		balanceQueryHint: 'When the rate endpoint has no valid balance, the backend queries this endpoint. The web page does not need to stay open.',
		balanceAutoProbe: 'Automatically probe balance/usage',
		balanceAutoProbeHint: 'Only opted-in accounts are queried in the background. The global switch stops every scheduled query.',
		queryConfigJSON: 'Usage query configuration (standard JSON)',
		queryConfigHint: 'Supports GET/POST, query, headers, JSON body, variables, and fixed field mappings. JavaScript is never executed.',
		queryConfigHelp: 'Show variable and mapping help',
		queryConfigScope: 'Define only what to request from the upstream and which response fields map to local standard fields. The system performs the request and parsing; do not enter JavaScript, functions, or expressions.',
		queryConfigVariables: 'Built-in names: baseUrl (exact Base URL), rootUrl (site root without terminal /v1), apiKey (model API key), accessToken (New API user PAT), and userId (legacy New-Api-User). Reference names with double braces in JSON. Add custom variables under variables; secret: true stores the value as a sensitive credential.',
		queryConfigMapping: 'Local fields: is_valid, invalid_message, plan_name, remaining (balance), used, total, unit (USD/CNY), and extra. Numeric mappings support multiplier/divisor; total also supports remaining_plus_used.',
		testQuery: 'Test usage query',
		balanceCurrency: 'Upstream balance currency',
		balanceCurrencyAuto: 'Automatic (use API value)',
		balanceCurrencyHint: 'Overrides only an incorrect upstream display unit; it does not convert the balance amount.',
		balanceAccessToken: 'User PAT (system access token)',
		balanceUserID: 'User ID (optional legacy)',
		balanceUserIDHint: 'Current New API versions do not need this. Set it only when a legacy deployment still validates New-Api-User.',
		balanceCredentialsRequired: 'New API balance queries require a user PAT, not a model API key',
        subscription: 'Subscription: #{id}',
        subscriptionShort: 'Subscription #{id}',
        upstreamUsage: 'Upstream usage: {requests} requests / {tokens} tokens',
        usagePeriod: 'Usage period: {start} to {end}',
        requestsShort: '{value} req',
        tokensShort: '{value} tokens',
        autoDisableState: 'Auto-disable on zero balance:',
        autoDisabledAt: 'Scheduling stopped for zero balance: {value}',
