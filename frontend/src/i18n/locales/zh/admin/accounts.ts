export default {
    accounts: {
      ...
      upstreamBilling: {
        trustWarning: '倍率、余额、订阅及用量均由上游站点针对当前 API Key 自行声明。Sub2API 无法验证其是否与实际扣费一致；请结合上游账单自行核验。',
        ...
        balance: '余额：{value}',
        balanceShort: '余额 {value}',
        balanceUnknown: '余额未知',
        billingMode: '计费类型：{value}',
        billingModeSubscription: '订阅计费',
        billingModeBalance: '余额计费',
		balanceQueryMode: '余额查询方式',
		balanceQueryAuto: '自动兼容（本系统 / 通用接口）',
		balanceQueryGeneric: '通用 /user/balance',
		balanceQueryNewAPI: 'New API /api/user/self',
		balanceQueryCustom: '自定义 JSON 模板',
		balanceQueryHint: '倍率接口未返回有效余额时，后端会使用这里配置的接口查询；无需打开网页。',
		balanceAutoProbe: '自动探测余额/用量',
		balanceAutoProbeHint: '仅此账号开启后才参与后台余额查询；全局开关关闭时所有账号停止定时查询。',
		queryConfigJSON: '用量查询配置（标准 JSON）',
		queryConfigHint: '支持 GET/POST、query、headers、JSON body、变量和固定字段映射；不执行 JavaScript。',
		queryConfigHelp: '查看变量和映射说明',
		queryConfigScope: '这里只定义“请求上游什么数据”和“把响应哪个字段映射到本地标准字段”，系统负责发请求和解析。不要填写 JavaScript、函数或表达式。',
		queryConfigVariables: '内置变量名：baseUrl（原始 Base URL）、rootUrl（去掉末尾 /v1 的站点根地址）、apiKey（模型 API Key）、accessToken（New API 用户 PAT）、userId（兼容旧版 New-Api-User）。在 JSON 中用双花括号引用；自定义变量写在 variables 中，secret: true 会保存到敏感凭据。',
		queryConfigMapping: '本地标准字段：is_valid、invalid_message、plan_name、remaining（余额）、used（已用）、total（总量）、unit（USD/CNY）、extra。数值可用 multiplier/divisor，total 还可用 remaining_plus_used 推导。',
		testQuery: '测试用量查询',
		balanceCurrency: '上游余额币种',
		balanceCurrencyAuto: '自动（采用接口返回值）',
		balanceCurrencyHint: '仅在上游返回的币种不正确时覆盖显示单位，不换算余额数值。',
		balanceAccessToken: '用户 PAT（系统访问令牌）',
		balanceUserID: '用户 ID（旧版可选）',
		balanceUserIDHint: '新版 New API 不需要此项；仅旧版仍校验 New-Api-User 请求头时填写。',
		balanceCredentialsRequired: 'New API 余额查询需要填写用户 PAT（不是模型 API Key）',
        subscription: '订阅号：#{id}',
        subscriptionShort: '订阅 #{id}',
        upstreamUsage: '上游用量：{requests} 次 / {tokens} Token',
        usagePeriod: '统计周期：{start} 至 {end}',
        requestsShort: '{value} 次',
        tokensShort: '{value} Token',
        autoDisableState: '余额不足自动下线：',
        autoDisabledAt: '已因余额不足停止调度：{value}',
