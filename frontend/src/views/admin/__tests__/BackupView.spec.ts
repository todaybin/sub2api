...
describe('admin BackupView 分卷备份', () => {
  ...
  it('本地备份使用认证下载接口并释放对象 URL', async () => {
    listBackups.mockResolvedValue({
      items: [{ ...baseRecord('local'), storage_type: 'local' }],
    })
    const blob = new Blob(['backup'])
    downloadBackup.mockResolvedValue(blob)
    const createObjectURL = vi.fn(() => 'blob:local-backup')
    ...
    expect(downloadBackup).toHaveBeenCalledWith('local')
