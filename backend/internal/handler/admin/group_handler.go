...
type CreateGroupRequest struct {
	...
	Description               string                        `json:"description"`
	Platform                  string                        `json:"platform" binding:"omitempty,oneof=anthropic openai gemini antigravity grok kimi zhipu deepseek minimax codebuddy opencode_go composite"`
	RateMultiplier            float64                       `json:"rate_multiplier"`
	RateMode                  string                        `json:"rate_mode" binding:"omitempty,oneof=fixed dynamic"`
...
type CompositeRouteRequest struct {
	...
	TargetPlatform string `json:"target_platform" binding:"required,oneof=anthropic openai gemini antigravity grok kimi zhipu deepseek minimax codebuddy opencode_go"`
...
func (h *GroupHandler) Create(c *gin.Context) {
	...
	group, err := h.adminService.CreateGroup(c.Request.Context(), &service.CreateGroupInput{
		...
		DynamicRateMarkupPercent:        req.DynamicRateMarkupPercent,
