...
// NormalizeOpenAICompatiblePlatform 保留 grok、CodeBuddy 与国产 OpenAI 兼容供应商
// （kimi/zhipu/deepseek）的原值，其他值一律归一为 openai。调度器据此对账号与
// 请求做精确平台匹配：CodeBuddy 分组请求只命中 CodeBuddy 账号。
...
func NormalizeOpenAICompatiblePlatform(platform string) string {
	switch platform {
	case PlatformGrok, PlatformKimi, PlatformZhipu, PlatformDeepseek, PlatformMiniMax, PlatformCodeBuddy, PlatformOpenCodeGo:
		return platform
	default:
