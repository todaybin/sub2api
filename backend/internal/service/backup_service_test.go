...
func TestBackupService_LocalBackupRemainsSingleFileAndUsesAuthenticatedDownload(t *testing.T) {
	repo := newMockSettingRepo()
	svc := newTestBackupService(repo, &mockDumper{dumpData: entropyBackupFixture(512)}, newMockObjectStore())
	root := t.TempDir()
	svc.localBackupDirectory = root
	svc.partSizeBytes = 1
	_, err := svc.UpdateStorageConfig(context.Background(), BackupStorageConfig{StorageType: BackupStorageTypeLocal})
	require.NoError(t, err)

	record, err := svc.CreateBackup(context.Background(), "manual", 14)
	require.NoError(t, err)
	require.Empty(t, record.Parts)
	backupPath := filepath.Join(root, filepath.FromSlash(record.StorageKey))
	require.FileExists(t, backupPath)

	_, err = svc.GetBackupDownloadURL(context.Background(), record.ID)
	require.ErrorContains(t, err, "local backups must use the authenticated download endpoint")

	openedRecord, body, err := svc.OpenBackup(context.Background(), record.ID)
	require.NoError(t, err)
	require.Equal(t, record.ID, openedRecord.ID)
