...
func (s *localBackupStore) UploadFile(ctx context.Context, key string, filePath string, contentType string) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("open local backup source: %w", err)
	}
...
func (s *BackupService) prepareBackupStore(ctx context.Context, fileName string) (BackupObjectStore, *BackupS3Config, string, string, string, string, error) {
	...
	if storageCfg.StorageType == BackupStorageTypeLocal {
		...
		return store, nil, BackupStorageTypeLocal, dateKey, store.root, "", nil
	}
	...
	return store, s3Cfg, BackupStorageTypeS3, s.buildS3Key(s3Cfg, fileName), "", provider, nil
}
...
func (s *BackupService) CreateBackup(ctx context.Context, triggeredBy string, expireDays int) (*BackupRecord, error) {
	...
	objectStore, s3Cfg, storageType, storageKey, storageRoot, storageProvider, err := s.prepareBackupStore(ctx, fileName)
	if err != nil {
		return nil, err
	}
...
func (s *BackupService) uploadBackupArchive(ctx context.Context, record *BackupRecord, objectStore BackupObjectStore, cfg *BackupS3Config, archivePath string) error {
	...
	if cfg == nil || info.Size() <= partSize {
		if key == "" {
			return errors.New("backup object key is empty")
		}
		if _, err := objectStore.UploadFile(ctx, key, archivePath, "application/gzip"); err != nil {
...
func (s *BackupService) StartRestore(ctx context.Context, backupID string) (*BackupRecord, error) {
	...
	go func() {
		...
		s.executeRestore(record, objectStore, key)
	}()
...
func (s *BackupService) executeRestore(record *BackupRecord, objectStore BackupObjectStore, key string) {
	ctx, cancel := context.WithTimeout(s.bgCtx, 30*time.Minute)
	defer cancel()
...
func (s *BackupService) GetBackupDownloadURL(ctx context.Context, backupID string) (BackupDownloadResponse, error) {
	...
	if record.StorageType == BackupStorageTypeLocal {
		return download, infraerrors.BadRequest("LOCAL_BACKUP_DOWNLOAD_REQUIRED", "local backups must use the authenticated download endpoint")
	}
	objectStore, key, err := s.storeForRecord(ctx, record)
	if err != nil {
		return download, err
...
func (s *BackupService) OpenBackup(ctx context.Context, backupID string) (*BackupRecord, io.ReadCloser, error) {
	...
	if len(record.Parts) > 0 {
		return nil, nil, infraerrors.BadRequest("SPLIT_BACKUP_DOWNLOAD_REQUIRED", "split backups must use the part download URLs")
	}
