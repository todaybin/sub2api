...
func (s *GatewayService) calculateRecordUsageCost(
	...
	if result.ImageCount > 0 {
		if resolved := s.resolveChannelPricing(ctx, billingModel, apiKey); resolved != nil && resolved.Mode == BillingModeToken {
			cost := s.calculateTokenCost(ctx, result, apiKey, billingModel, multiplier, pricingAt)
			baseCost := s.calculateTokenCost(ctx, result, apiKey, billingModel, 1, pricingAt)
	...
	baseTokenCost := s.calculateTokenCost(ctx, result, apiKey, billingModel, 1, pricingAt)
